  ...
  const [saved, setSaved] = useState(false);
      ...
      const generateOfficeStaffId = async () => {
        const year = new Date().getFullYear().toString().slice(-2);
        const roleLetter = "O"; // 🔥 Office Staff
      
        const snap = await getDocs(
          collection(db, "users", adminUid, "office_staffs")
        );
      
        const ids = snap.docs.map(d => d.data().staffId || "");
        ...
        if (editData) {
          ...
          // 🔥 CREATE → generate ID
          const loadId = async () => {
            const id = await generateOfficeStaffId();
            setForm(prev => ({ ...prev, staffId: id }));
          };
